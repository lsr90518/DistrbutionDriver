...
-(id) initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, frame.size.height - 70)];
        ...
        distributionProcessLabel.text = @"運送中";
        ...
        acceptView = [[UIView alloc]initWithFrame:CGRectMake(0, frame.size.height - 70, frame.size.width, 70)];
        [acceptView setBackgroundColor:[UIColor colorWithRed:238.0/255.0 green:238.0/255.0 blue:238.0/255.0 alpha:1]];
        acceptButton = [[UIButton alloc]initWithFrame:CGRectMake(10, 10, frame.size.width - 20, 50)];
        [acceptButton setTitle:@"受ける" forState:UIControlStateNormal];
        [acceptButton setBackgroundColor:[UIColor colorWithRed:238.0/255.0 green:160.0/255.0 blue:24.0/255.0 alpha:1]];
        [acceptButton addTarget:self  action:@selector(acceptButtonTouched) forControlEvents:UIControlEventTouchUpInside];
        acceptButton.titleLabel.font = [UIFont fontWithName:@"HiraKakuProN-W3" size:16];
...
-(void) setDriverReviewContent:(MDReview *)review{
    [takeButton setTitle:@"依頼者の評価待ち" forState:UIControlStateNormal];
    [takeButton removeTarget:self action:@selector(takeButtonTouched:) forControlEvents:UIControlEventTouchUpInside];
}
...
-(void) matchButtonTouched:(MDSelect *)button{
    if([self.delegate respondsToSelector:@selector(matchButtonPushed:)]){
        [self.delegate matchButtonPushed:button];
    }
}
...
//    statusButton
    [takeButton setTitle:@"荷物預かったらここをタッチ" forState:UIControlStateNormal];
    [takeButton setTitleColor:[UIColor colorWithRed:226.0/255.0 green:0 blue:0 alpha:1] forState:UIControlStateNormal];
    takeButton.layer.borderColor = [UIColor colorWithRed:226.0/255.0 green:0 blue:0 alpha:1].CGColor;
...
-(void) deliveryPackageButton{
    //    statusButton
    [takeButton setTitle:@"配達完了したらここをタッチ" forState:UIControlStateNormal];
    [takeButton setTitleColor:[UIColor colorWithRed:226.0/255.0 green:138.0/255.0 blue:0 alpha:1] forState:UIControlStateNormal];
    takeButton.layer.borderColor = [UIColor colorWithRed:226.0/255.0 green:138.0/255.0 blue:0 alpha:1].CGColor;
    [takeButton addTarget:self action:@selector(takeButtonTouched:) forControlEvents:UIControlEventTouchUpInside];
    takeButton.tag = 2;
    [_scrollView addSubview:takeButton];
...
-(void) reviewPakcageButton{
    [takeButton setTitle:@"依頼者を評価" forState:UIControlStateNormal];
...
-(void) takeButtonTouched:(UIButton *)button {
    if([self.delegate respondsToSelector:@selector(takeButtonPushed:)]){
        [self.delegate takeButtonPushed:button];
    }
}
...
-(void) seeCancelPackage:(MDPackage *)package{
    ...
    //camera
    cameraButton = [[UIButton alloc]initWithFrame:CGRectMake(10, _scrollView.frame.origin.y + 10,  _scrollView.frame.size.width-20, (_scrollView.frame.size.width-20)*0.6)];
    [cameraButton setBackgroundColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:0.4]];
    [cameraButton addTarget:self action:@selector(cameraButtonTouched) forControlEvents:UIControlEventTouchUpInside];
    [cameraButton sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",package.image]] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"cargo"]];
    cameraButton.imageView.contentMode = UIViewContentModeScaleAspectFill;
    [_scrollView addSubview:cameraButton];
    
    reviewWell = [[MDSelectRatingWell alloc]initWithFrame:CGRectMake(10, cameraButton.frame.origin.y + cameraButton.frame.size.height + 10, self.frame.size.width - 20, 100)];
    [reviewWell.selectRating setNoArrow];
    [reviewWell.selectRating.starLabel setRating:[package.userReview.star intValue]];
    [reviewWell setContentText:package.userReview.text];
    [_scrollView addSubview:reviewWell];
